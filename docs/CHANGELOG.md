...
- **Mobile performance** — Screenshots converted from PNG (~68 MB) to WebP (~1.6 MB, max 1600px). Fontshare loaded via `<link>` + preconnect (no blocking `@import`). `AppReadyGate` uses `requestAnimationFrame` instead of a 300ms delay. `viewport` export added. Preload of gallery neighbors skipped on touch/`saveData`. Certificate PDF iframes replaced with a stable fallback on coarse-pointer devices; desktop iframes use eager load for the active slide. Gallery/lightbox nav targets ≥44px. About parallax gated to desktop (`min-width: 1024px`). Navbar caches section nodes on scroll. Projects liquid blobs respect `prefers-reduced-motion`. `--color-primary-glow` token defined. Desktop: shared image decode cache, preload on modal/lightbox expand, cached-image skeleton skip, `content-visibility` on inactive gallery slides.
- **Dead code** — Removed unused `react-icons`, `Spinner`, `src/types/index.ts`, and `components/common/index.ts` barrel. Dropped no-op `gsap.registerPlugin(useGSAP)` calls.
